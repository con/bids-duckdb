# `bids_duckdb` developer documentation
...
# install bids_duckdb as an editable package
...
The second is to use `tox`, which can be installed separately (e.g. with `pip install tox`), i.e. not necessarily inside the virtual environment you use for installing `bids_duckdb`, but then builds the necessary virtual environments itself by simply running:
...
cd $(mktemp -d bids_duckdb.XXXXXX)
git clone git@github.com:con/bids_duckdb .
...
Visit
[https://test.pypi.org/project/bids_duckdb](https://test.pypi.org/project/bids_duckdb)
and verify that your package was uploaded successfully. Keep the terminal open, we'll need it later.

In a new terminal, without an activated virtual environment or an env directory:

```shell
cd $(mktemp -d bids_duckdb-test.XXXXXX)
...
--extra-index-url https://pypi.org/simple bids_duckdb
```
...
Don't forget to also make a [release on GitHub](https://github.com/con/bids_duckdb/releases/new).GitHub-Zenodo integration will also trigger Zenodo into making a snapshot of your repository and sticking a DOI on it.